...
export async function fetchGitHubProfile(username: string): Promise<CandidateProfile> {
  ...
  try {
    ...
    const [userRes, reposRes] = await Promise.all([
      ...
      axios.get<GitHubRepo[]>(`${GITHUB_API}/users/${username}/repos?sort=stars&per_page=10`, {
        headers,
      }),
    ]);
    ...
    const languagePromises = repos.slice(0, 5).map(async (repo) => {
      try {
        const langRes = await axios.get<Record<string, number>>(repo.languages_url, { headers });
        return langRes.data;
      } catch {
        return {};
      }
...
export function extractGitHubUsername(input: string): string | null {
  // Handle full URL
  const urlPattern = /github\.com\/([a-zA-Z0-9](?:[a-zA-Z0-9]|-(?=[a-zA-Z0-9])){0,38})/i;
